			...
			>
				<UTooltip :text="userData?.name">
					<UAvatar
						...
						:alt="userData?.name"
						class="border-2 border-accent50 dark:border-dark-accent50 w-7 h-7"
					/>
				</UTooltip>
			</div>
		</header>
		...
		<section @click.stop="isMenuOpen = false" class="lg:px-5">
			...
			>
				<article
					...
					:key="voyage.id"
					class="relative rounded-xl bg-white dark:bg-dark-background100 shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden border border-gray-100 dark:border-dark-border100 min-w-[200px] sm:min-w-[400px] md:min-w-[350px] lg:min-w-[350px] xl:min-w-[450px] mx-auto"
...
const getFirstImageUrl = (imageUrls: string | string[]): string | undefined => {
	try {
